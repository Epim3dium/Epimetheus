...
LayerInfo getLayerValues(Slice<Entity, Parent> slice) {
    ...
    for(int layer = 0; layer < result.max_layer; layer++) {
        if(result.path[last_layer_member_index[layer]] == (size_t)-1) {
            result.path[last_layer_member_index[layer]] = first_layer_member_index[layer + 1]; 
        }
    }
...
void updateLocalTransforms(Slice<Entity, Position, Rotation, Scale, LocalTransform> slice) {
...
void updateParentTransformByHierarchy(Slice<Entity, LocalTransform, GlobalTransform> slice, const HierarchyGroup& hierarchy, LayerInfo layer_info) {
    size_t to_update = 0;
    int iter=0;
...
struct System {
    ...
    void add(Entity id, std::string name, Position p, Rotation r, Parent parent, sf::Color color) {
        name_table[id] = name;
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        updateLocalTransforms(sys.transforms.slice<Position, Rotation, Scale, LocalTransform>());
        updateParentTransformByHierarchy(sys.transforms.slice<LocalTransform, GlobalTransform>(), sys.hierarchy, info);
        ...
        }
        {
            ...
            auto slider2D = [&](std::string name, sf::Vector2f* vec, sf::Vector2f min, sf::Vector2f max, float scale) {
            ...
            for(auto [e, name] : sys.name_table) {
                ...
                auto& ref_pos = *sys.transforms.getComponent<Position>(e).value();
                auto height = window.getView().getSize().y;
                ref_pos.y = height - ref_pos.y;
                slider2D(name + " pos", &ref_pos, {0.f, 0.f} , {window.getView().getSize()}, 1.f);
                ref_pos.y = height - ref_pos.y;
                
                auto& ref_scale = *sys.transforms.getComponent<Scale>(e).value();
                slider2D(name + " scale", &ref_scale, {-2.f, -2.f} , {2.f, 2.f}, 1.f);
                
                auto& ref_rotate = *sys.transforms.getComponent<Rotation>(e).value();
                ImGui::SliderFloat((name + " rotation").c_str(), &ref_rotate.val, 0.f, 360.f);
                ImGui::EndChild();
...
ImVec2 operator*(float f, ImVec2 vec) {
