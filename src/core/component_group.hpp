...
class ComponentGroup : private Group<Enum> {
    ...
    void m_update(std::vector<Enum> identifiers, std::function<void(EntityID, Types...)> update_func) {
        ...
        for(size_t i = 0; i < size(); i++) {
            update_func(m_entityID_list[i], {( reinterpret_cast<base_type<Types>*>(pointers[idx++]))[i] }...);
            idx = 0;
        }
    }
public:
    ...
    size_t size() const {
        return Group<Enum>::m_instances;
    }
    template<class T, class ...Rest>
    void push_back(EntityID entityid, T v, Rest ... vals) {
        m_entityID_to_index_map.insert_or_assign(entityid, size());
        m_entityID_list.push_back(entityid);
        Group<Enum>::push_back(v, vals...);
    }
    void swap(size_t idx1, size_t idx2) {
        std::swap(m_entityID_to_index_map.at(m_entityID_list[idx1]), m_entityID_to_index_map.at(m_entityID_list[idx2]));
    ...
    T& get(Enum variable, size_t index) {
        return Group<Enum>::template get<T>(variable, index);
    }
    template<class T>
    inline std::optional<std::reference_wrapper<T>> getByID(Enum variable, EntityID id) {
        auto itr = m_entityID_to_index_map.find(id);
        if(itr == m_entityID_to_index_map.end()) {
            return {};
        }
        return get<T>(variable, itr->second);
    }
...
void updateMatching(ComponentGroup<Enum1>& g1, ComponentGroup<Enum2>& g2, std::vector<Enum1> ids1, std::vector<Enum2> ids2, Func update_func) 
{
    auto intersection = IntersectMaps(g1.getEntityIDToIndexMap(), g2.getEntityIDToIndexMap());
    g1.moveFoward(intersection);
    g2.moveFoward(intersection);
    helper::updateMatching(intersection.size(), g1, g2, ids1, ids2, std::function(std::forward<Func>(update_func)));
...
public:
    ...
    ComponentGroup::pointer create() {
        std::unique_ptr<ComponentGroup> result{ new ComponentGroup(m_init_values, m_identifiers)};
