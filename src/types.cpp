...
#include <vector>
namespace epi {
std::vector<ConvexPolygon> triangulateEarClipping(const std::vector<vec2f>& points) {
    ...
    while (result.size() != points.size() - 2) {
        ...
        for (int i = 0; i < tmp.size(); i++) {
            ...
            result.push_back(
                ConvexPolygon::CreateFromPoints({first, mid, last}));
...
static std::vector<ConvexPolygon>
partitionConvexHertelMehlhorn(std::vector<ConvexPolygon> triangles) {
    std::vector<ConvexPolygon> result;
    ...
    for (auto iter1 = triangles.begin(); iter1 != triangles.end(); iter1++) {
        ...
        for (i11 = 0; i11 < poly1->getVertecies().size(); i11++) {
            ...
            result.push_back(ConvexPolygon::CreateFromPoints(newpoly));
...
partitionConvex(const std::vector<ConvexPolygon>& polygons) {
